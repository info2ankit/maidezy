...
import { X, Star, Buildings, CalendarDot, ClockAfternoon } from '@phosphor-icons/react'
          ...
          <div className="flex items-start gap-4">
            ...
            {worker.photoUrl ? (
              <img
                src={worker.photoUrl}
                alt={worker.name}
                className="w-24 h-24 rounded-2xl object-cover shadow-lg border-2 border-white shrink-0"
              />
            ) : (
              <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center shadow-lg border-2 border-white shrink-0">
            ...
            <div className="flex-1 pt-1 min-w-0">
              <h2 className="font-heading font-bold text-gray-900 text-xl leading-tight">
                {worker.name}
              </h2>
                    ...
                    <div className="flex gap-0.5">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          key={s}
                          size={14}
                          weight={s <= roundedRating ? 'fill' : 'regular'}
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                ...
                <span
                  className={`font-body text-xs rounded-full px-2.5 py-0.5 font-medium ${
                    worker.isAvailable
          ...
          <div className="px-5 py-5 border-t border-gray-100">
            ...
            <div className="space-y-2">
              {worker.pricing.map((p) => {
                ...
                const Icon = st?.icon
                const colors = SERVICE_COLORS[p.serviceTypeId] ?? {
                  icon: 'text-gray-500',
                  bg: 'bg-gray-100',
                }
                return (
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="space-y-1">
                  {worker.shifts.map((s, i) => (
                    <div
                      key={i}
                      className="inline-flex items-center bg-gray-50 rounded-xl px-3 py-1.5 mr-2"
                    >
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {societyNames.map((name) => (
                  <span
                    key={name}
                    className="font-body text-xs bg-blue-50 text-blue-700 border border-blue-100 rounded-full px-3 py-1 font-medium"
                  >
                    {name}
                  </span>
                ))}
